...
function replayCandidateId(row) { return String(row?.candidate_id || row?.trade_id || 'unknown'); }
...
function replayAnalyticsSummary(events) { const rows = arr(events); const timestamps = rows.map(replayTimestampNumber).filter((value) => value !== null); const scores = rows.map(replayQualityScore).filter((value) => value !== null); return { candidateCount: new Set(rows.map(replayCandidateId)).size, eventCount: rows.length, statusDistribution: replayDistribution(rows.map(replayStatus)), strategyDistribution: replayDistribution(rows.map(replayStrategy)), timeWindowMin: timestamps.length ? Math.min(...timestamps) : null, timeWindowMax: timestamps.length ? Math.max(...timestamps) : null, bestQualityScore: scores.length ? Math.max(...scores) : null, worstQualityScore: scores.length ? Math.min(...scores) : null }; }
