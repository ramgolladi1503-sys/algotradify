...
function App() {
  ...
  const [state, setState] = useState({ health: null, preflight: null, snapshot: null, opportunities: [], candidateTruth: [], opportunityLayer: null, executionReadiness: [], tradeQuality: [], topExecutable: null, executionSafety: null, dryRunExecution: null, dryRunExport: null, fillLifecycle: null, outcomeReplay: null });
  useEffect(() => savePersistedPreferences({ filters, replayCandidateId, operatorView }), [filters, replayCandidateId, operatorView]);
  async function fetchJson(path) { const r = await fetch(`${API_BASE}${path}`); if (!r.ok) throw new Error(path); return r.json(); }
  async function fetchControlTower(candidateOverride = replayCandidateId) { const candidateQuery = candidateOverride.trim() ? `?candidate_id=${encodeURIComponent(candidateOverride.trim())}` : ''; const requests = [['health', '/runtime/health'], ['preflight', '/runtime/preflight'], ['snapshot', '/runtime/snapshot'], ['opportunities', '/opportunities?limit=20'], ['candidateTruth', '/candidate-truth?limit=20'], ['opportunityLayer', '/opportunity-layer?limit=20'], ['executionReadiness', '/execution-readiness?limit=20'], ['tradeQuality', '/trade-quality?limit=20'], ['topExecutable', '/top-executable?limit=20'], ['executionSafety', '/execution-safety?limit=20'], ['dryRunExecution', '/dry-run-execution?limit=20'], ['dryRunExport', '/dry-run-execution/export?limit=20'], ['fillLifecycle', '/fill-lifecycle'], ['outcomeReplay', `/outcome-replay${candidateQuery}`]]; const results = await Promise.allSettled(requests.map(([, p]) => fetchJson(p))); const next = {}; results.forEach((r, i) => { if (r.status === 'fulfilled') next[requests[i][0]] = r.value; }); setState((p) => ({ ...p, ...next })); }
  ...
  function resetToDefaultView() { clearPersistedPreferences(); setOperatorView('default'); setFilters(DEFAULT_FILTERS); setReplayCandidateId(''); }
  ...
  return <div style={{ background: '#0b1220', color: '#e8eefc', minHeight: '100vh', padding: 20 }}><h1>Algotradify Control Tower</h1><p style={muted}>Runtime health, persisted UI preferences, execution safety, dry-run evidence, export preview, filters, analytics charts, replay drilldowns, and lifecycle evidence.</p>
