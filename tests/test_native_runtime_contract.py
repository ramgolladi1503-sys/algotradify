...
def _make_native_source_root(path: Path, *, wrapper_main: bool = False) -> Path:
...
def test_native_runtime_source_root_detects_promoted_main(tmp_path, monkeypatch):
...
def test_wrapper_main_still_reports_pending_main_promotion_in_fixture(tmp_path, monkeypatch):
    contract = importlib.import_module("runtime_contract")
    _clear_runtime_env(monkeypatch)
...
def test_preflight_reports_native_after_main_promotion(tmp_path, monkeypatch):
    contract = importlib.import_module("runtime_contract")
    _clear_runtime_env(monkeypatch)
    root = _make_native_source_root(tmp_path / "algotradify", wrapper_main=False)
    monkeypatch.setenv("ALGOTRADIFY_REQUIRE_NATIVE_RUNTIME", "true")
    ...
    assert result["native_main_promoted"] is True
    assert result["external_runtime_used"] is False
    assert any(check["name"] == "native_runtime_main.promoted" and check["status"] == "PASS" for check in result["checks"])


def test_default_runtime_resolution_prefers_native_root_after_pr5(tmp_path, monkeypatch):
    contract = importlib.import_module("runtime_contract")
    _clear_runtime_env(monkeypatch)
    root = _make_native_source_root(tmp_path / "algotradify", wrapper_main=False)
    _make_external_runtime(root / "core_bot")
